    ...
    <div className="container">
      <header>
        ...
        <div className="pixel-title">
          WHAT GYM LEADER<br />TYPE ARE YOU?
        </div>
      ...
      {screen === 'intro' && (
        <div className="intro-card">
          ...
          <p style={{ color: 'var(--accent)', fontWeight: 800 }}>
            No Pokémon knowledge needed. Just be yourself. ✨
          </p>
      ...
      {screen === 'result' && (() => {
        ...
        return (
          <div className="card result-card" style={{ borderTop: `5px solid ${data.color}` }}>
            ...
            <h2 className="result-type" style={{ color: data.color }}>{data.type}</h2>
            <h3 className="result-headline">{data.headline}</h3>
            <span className="type-tag" style={{ background: data.color }}>{data.tag}</span>
            <p className="result-desc">{data.desc}</p>

            <div className="traits">
              {data.traits.map((trait) => (
                <span key={trait} className="trait" style={{ color: data.color }}>
                  {trait}
                </span>
              ))}
            </div>
